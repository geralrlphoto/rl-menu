          ...
          {projeto.entregas.map((e, i) => (
            <div key={i}
              ...
              <div className="flex items-start gap-4">
                <span className="text-[9px] font-mono text-white/15 mt-0.5">{String(i + 1).padStart(2, '0')}</span>
                <div>
                  <p className="text-[11px] tracking-[0.25em] text-white/65 uppercase font-medium">{e.titulo}</p>
                  <p className="text-[9px] tracking-[0.15em] text-white/25 mt-1">{e.formato} · {e.duracao}</p>
                </div>
              </div>
              {e.estado === 'disponivel' && e.linkUrl ? (
                <a href={e.linkUrl} target="_blank" rel="noopener noreferrer"
                  className="shrink-0 border border-white/20 bg-white/5 hover:bg-white/10 px-4 py-2 text-[9px] tracking-[0.3em] text-white/60 uppercase transition-colors">
                  Descarregar →
                </a>
              ) : (
                <span className="shrink-0 text-[8px] tracking-[0.35em] text-white/20 uppercase">Pendente</span>
              )}
            </div>
          ))}
        </div>
