      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-10 py-10">
        ...
        {!isEditing && (
          <div className="mb-10 border border-white/[0.07] bg-white/[0.02] px-6 py-6 flex flex-col gap-4">
            <p className="text-[12px] tracking-[0.5em] text-white/30 uppercase">As tuas entregas</p>
            <p className="text-[16px] font-light text-white/55 leading-relaxed tracking-wide">
        ...
        {!isEditing && (
          <div className="mb-8 border border-white/[0.08] bg-white/[0.02] px-6 py-5">
            <p className="text-[12px] tracking-[0.5em] text-white/30 uppercase mb-4">Estado das Entregas</p>
                ...
                return (
                  <div key={i} className="flex items-center justify-between gap-3 py-2 border-t border-white/[0.04] first:border-t-0 first:pt-0">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <span className="text-[14px] font-light text-white/50 truncate">{e.titulo}</span>
                    </div>
                    <span className={`text-[12px] tracking-[0.3em] uppercase shrink-0 ${disponivel ? 'text-emerald-400/60' : 'text-red-400/45'}`}>
              ...
              <div key={i} className="mb-5">
                ...
                <div className={`border px-6 py-5 transition-colors ${
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <span className="text-[13px] font-mono text-white/15 mt-0.5 shrink-0">{String(i + 1).padStart(2, '0')}</span>
                      <div className="flex-1 min-w-0">

                        <div className="flex items-center gap-2 mb-0.5">
                          {e.tipo && !isEditing && (
                            <span className="text-[16px] opacity-60">{TIPO_ICON[e.tipo] ?? '📁'}</span>
                          )}
                          {isEditing ? (
                            <select
                              value={e.tipo ?? ''}
                              onChange={ev => updateEntrega(i, 'tipo', ev.target.value)}
                              className="bg-[#04080f] border border-white/[0.08] text-[14px] text-white/45 px-2 py-1 focus:outline-none focus:border-white/20 mr-2"
                            >
                              {TIPO_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                            </select>
                          ) : e.tipo ? (
                            <span className="text-[12px] tracking-[0.2em] text-white/25 uppercase">{e.tipo}</span>
                          ) : null}
                        </div>
                        ...
                        <EditableField
                          ...
                          className="text-[14px] tracking-[0.25em] text-white/65 uppercase font-medium block"
                          placeholder="Título"
                        />
                        <div className="flex items-center gap-2 mt-1">
                          <EditableField
                            ...
                            className="text-[13px] tracking-[0.15em] text-white/25 block"
                        ...
                        {isEditing && (
                          <div className="mt-3 flex items-center gap-2 border border-white/[0.08] bg-white/[0.02] px-3 py-2">
                            <span className="text-[16px] text-white/20 shrink-0">🔗</span>
                            <input
                              value={e.linkUrl ?? ''}
                              onChange={ev => updateEntrega(i, 'linkUrl', ev.target.value)}
                              placeholder="URL de download (https://...)"
                              className="flex-1 bg-transparent text-[14px] text-white/50 placeholder:text-white/15 focus:outline-none"
                            />
                            {e.linkUrl && (
                              <button onClick={() => updateEntrega(i, 'linkUrl', '')}
                                className="text-white/20 hover:text-white/50 text-[14px] shrink-0">✕</button>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="shrink-0 flex flex-col items-end gap-2">
                      {!isEditing ? (
                        ...
                        <button onClick={() => removeEntrega(i)}
                          className="text-[12px] tracking-[0.3em] text-red-400/50 hover:text-red-400/80 uppercase transition-colors">
                          Remover
                        </button>
                      )}
                    </div>
                  ...
                  {!isEditing && (
                    <div className="mt-4 pt-4 border-t border-white/[0.04] flex flex-wrap items-center justify-between gap-3">
                      <p className="text-[12px] tracking-[0.3em] text-white/18 uppercase">
                        {feedbacks.length > 0
                      ...
                      {!isAdmin && (
                        temUrl ? (
                          feedbackAberto !== i && (
                            <button
                              ...
                              className="shrink-0 inline-flex items-center gap-2 border border-emerald-400/40
                                         ...
                                         text-[12px] tracking-[0.35em] text-emerald-400/80 uppercase transition-colors"
                            >
                              ◎ Dar Feedback
                            </button>
                          )
                          ...
                          <span className="shrink-0 inline-flex items-center gap-2 border border-red-500/20
                                           bg-red-500/[0.03] px-4 py-2 text-[12px] tracking-[0.35em]
                      ...
                      {isAdmin && (
                        temUrl ? (
                          <div className="flex flex-col items-end gap-1.5">
                            <div className="flex items-center gap-2 flex-wrap justify-end">
                              <span className="shrink-0 inline-flex items-center gap-2 border border-emerald-400/20
                                               bg-emerald-400/[0.04] px-3 py-2 text-[12px] tracking-[0.35em]
                              ...
                              <button
                                ...
                                className="shrink-0 inline-flex items-center gap-2 border border-white/18
                                           ...
                                           px-3 py-2 text-[12px] tracking-[0.3em] text-white/35 hover:text-white/65
                          ...
                          <span className="shrink-0 inline-flex items-center gap-2 border border-white/[0.07]
                                           bg-white/[0.02] px-4 py-2 text-[12px] tracking-[0.35em]
                ...
                {!isAdmin && feedbackAberto === i && (
                  <div className="border-x border-b border-emerald-400/15 bg-emerald-400/[0.02] px-5 py-4">
                    <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-3">O teu feedback</p>
                    <textarea
                      value={feedbackTexto}
                      ...
                      rows={4}
                      className="w-full bg-white/[0.03] border border-white/[0.08] text-[16px] font-light text-white/60
                                 placeholder:text-white/15 px-4 py-3 focus:outline-none focus:border-white/20
                                 resize-none leading-relaxed"
                    />
                    {feedbackErro && (
                      <p className="text-[13px] text-red-400/70 mb-3 leading-relaxed">⚠ {feedbackErro}</p>
                    )}
                    <div className="flex items-center gap-3 mt-3">
                      <button
                        onClick={() => submitFeedback(i)}
                        disabled={enviandoFeedback || !feedbackTexto.trim()}
                        className="px-5 py-2 text-[12px] tracking-[0.35em] uppercase border border-emerald-400/40
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-[12px] tracking-[0.35em] uppercase text-white/25 hover:text-white/50 transition-colors"
                      >
                        Cancelar
                      </button>
                    </div>
                ...
                {(hasFeedbacks || isAdmin) && (
                  <div className="border-x border-b border-white/[0.05] bg-white/[0.01] px-5 py-4">
                    <p className="text-[12px] tracking-[0.45em] text-white/15 uppercase mb-4">Registos de Feedback</p>
                    {!hasFeedbacks && isAdmin && (
                      <p className="text-[13px] text-white/15 italic">Sem feedbacks do cliente ainda.</p>
                    )}
                        ...
                        <div key={fb.id} className="flex flex-col gap-2">
                          ...
                          <div className="border border-amber-400/15 bg-amber-400/[0.03] px-4 py-3">
                            <div className="flex items-center justify-between mb-2 gap-4">
                              <span className="text-[12px] tracking-[0.35em] text-amber-400/60 uppercase shrink-0">
                                ◎ Cliente
                              </span>
                              <div className="flex items-center gap-3">
                                <span className="text-[12px] text-white/18 font-mono">{formatDateTime(fb.criadoEm)}</span>
                                {isAdmin && (
                                  <button
                                    onClick={() => removerFeedback(i, fb.id)}
                                    title="Remover feedback"
                                    className="text-[13px] text-red-400/30 hover:text-red-400/70 transition-colors leading-none"
                                  >✕</button>
                                )}
                              </div>
                            </div>
                            <p className="text-[16px] font-light text-white/50 leading-relaxed">{fb.texto}</p>
                          </div>
                          ...
                          {fb.resposta ? (
                            <div className="border border-blue-500/18 bg-blue-500/[0.03] px-4 py-3 ml-5">
                              <div className="flex items-center justify-between mb-2 gap-4">
                                <span className="text-[12px] tracking-[0.35em] text-blue-400/70 uppercase shrink-0">
                                  ↩ RL PROD
                                </span>
                                <div className="flex items-center gap-3">
                                  <span className="text-[12px] text-white/18 font-mono">{formatDateTime(fb.resposta.criadoEm)}</span>
                                  {isAdmin && (
                                    <button
                              ...
                              <p className="text-[16px] font-light text-white/50 leading-relaxed">{fb.resposta.texto}</p>
                            </div>
                          ) : isAdmin ? (
                            /* Admin — responder */
                            respostaAberta === `${i}-${fb.id}` ? (
                              <div className="ml-5 border border-blue-500/20 bg-blue-500/[0.03] px-4 py-4">
                                <p className="text-[12px] tracking-[0.4em] text-blue-400/50 uppercase mb-3">Resposta</p>
                                <textarea
                                  value={respostaTexto}
                                ...
                                />
                                {respostaErro && (
                                  <p className="text-[13px] text-red-400/70 mb-3">⚠ {respostaErro}</p>
                                )}
                                <div className="flex gap-3">
                                  <button
                                    onClick={() => submitResposta(i, fb.id)}
                                    disabled={enviandoResposta || !respostaTexto.trim()}
                                    className="px-5 py-2 text-[12px] tracking-[0.35em] uppercase border border-blue-400/40
                              ...
                              <div className="ml-5">
                                <button
                                  ...
                                  className="text-[12px] tracking-[0.35em] uppercase text-blue-400/50 hover:text-blue-400/80
                            ...
                            <div className="ml-5 px-4 py-3 border border-white/[0.04] bg-white/[0.01]">
                              <p className="text-[12px] text-white/20 tracking-[0.25em]">⏳ Aguardar resposta da RL PROD</p>
                            </div>
                          )}
                        </div>
                      ))}
        ...
        {isEditing && (
          <button onClick={addEntrega}
            className="w-full border border-dashed border-white/15 hover:border-white/30 bg-white/[0.01]
                       hover:bg-white/[0.03] py-3 text-[12px] tracking-[0.4em] text-white/30 uppercase
        ...
        <div className="mb-6 flex items-start gap-3">
          <div className="flex items-center gap-4 flex-1 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] text-white/30">Com download ({entregasComUrl})</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-red-500/60 shrink-0" />
              <span className="text-[13px] text-white/30">Aguarda link ({projeto.entregas.length - entregasComUrl})</span>
            </div>
          </div>
        </div>
        ...
        {isAdmin && !isEditing && (
          <div className="mb-8 border border-white/[0.07] bg-white/[0.02] px-5 py-5 flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-[12px] tracking-[0.35em] text-white/35 uppercase mb-1">Notificar Cliente</p>
              <p className="text-[16px] font-light text-white/20 leading-relaxed">
                {notificado
            ...
            <button
              ...
              className={`shrink-0 px-5 py-2.5 text-[12px] tracking-[0.35em] uppercase border transition-colors
                ${notificado
        ...
        <div className="mb-12 border border-white/[0.05] bg-white/[0.01] px-5 py-4">
          <p className="text-[13px] tracking-[0.2em] text-white/20 leading-relaxed">
        ...
        <div className="mb-10 flex items-center gap-4">
          ...
          <span className="text-[12px] tracking-[0.5em] text-white/15 uppercase">Revisões</span>
          <div className="h-px flex-1 bg-white/[0.06]" />
        </div>

        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="border border-white/[0.07] bg-white/[0.02] px-4 py-5 text-center">
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-2">Incluídas</p>
            {isEditing ? (
          ...
          <div className="border border-white/[0.07] bg-white/[0.02] px-4 py-5 text-center">
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-2">Usadas</p>
            {isEditing ? (
          ...
          <div className="border border-white/[0.07] bg-white/[0.02] px-4 py-5 text-center">
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-2">Restantes</p>
