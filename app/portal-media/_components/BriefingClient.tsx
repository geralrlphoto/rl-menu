      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-10 py-10">
        ...
        <div className="flex items-end justify-between mb-10">
          ...
          {isAdmin && (
            <button
              onClick={addSessao}
              className="border border-dashed border-white/20 hover:border-white/40 px-4 py-2 text-[9px] tracking-[0.3em] text-white/30 hover:text-white/60 uppercase transition-colors shrink-0"
            >
              + Sessão
            </button>
          )}
        ...
        <div className="mb-10 border border-white/[0.07] bg-white/[0.02] px-6 py-6 flex flex-col gap-4">
          <p className="text-[9px] tracking-[0.5em] text-white/30 uppercase">O que é o Briefing?</p>
        ...
        {sessoes.length === 0 && (
          <div className="border border-dashed border-white/[0.07] px-6 py-14 text-center">
            <p className="text-[9px] tracking-[0.4em] text-white/20 uppercase mb-2">Sem briefings registados</p>
            {isAdmin && (
              <p className="text-[12px] font-light text-white/15">Clica em "+ Sessão" para adicionar o primeiro briefing</p>
            )}
          </div>
        )}
            ...
            <div key={sessao.id} className="border border-white/[0.08] bg-white/[0.02]">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-6 py-4 border-b border-white/[0.06]">
                ...
                <EditableDateField
                  ...
                  className="text-[9px] tracking-[0.2em] text-white/30 shrink-0"
                  placeholder="Data"
                />
              </div>
              ...
              <div className="px-6 py-5">
                <p className="text-[9px] tracking-[0.4em] text-white/20 uppercase mb-3">Resumo</p>
                {isEditing ? (
                  <textarea
                    value={sessao.resumo}
                    ...
                    rows={7}
                    className="w-full bg-white/[0.03] border border-white/[0.08] px-4 py-3 text-[13px] font-light text-white/55 leading-relaxed resize-none focus:outline-none focus:border-white/20 placeholder:text-white/15"
                  />
                ) : sessao.resumo ? (
                  <p className="text-[13px] font-light text-white/40 leading-relaxed whitespace-pre-wrap">{sessao.resumo}</p>
                ) : (
                  <p className="text-[12px] font-light text-white/15">Sem resumo registado.</p>
                )}
              </div>
                ...
                <>
                  <div className="px-6 py-4 border-t border-white/[0.06] flex flex-wrap items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className={`px-5 py-2 text-[9px] tracking-[0.35em] uppercase border transition-colors shrink-0
                        ${sessao.notificacaoEnviada
                  ...
                  <div className="px-6 py-3 border-t border-white/[0.04] flex items-center justify-end">
                    {confirmDelete === sessao.id ? (
                      <div className="flex items-center gap-3">
                        <span className="text-[12px] text-white/35 tracking-[0.2em]">Tens a certeza?</span>
                        <button
                          onClick={() => eliminar(sessao.id)}
                          className="px-4 py-1.5 text-[9px] tracking-[0.3em] uppercase border border-red-400/40 text-red-400/70 hover:border-red-400/70 hover:text-red-400 transition-colors"
                        >
                          Eliminar
                        </button>
                        <button
