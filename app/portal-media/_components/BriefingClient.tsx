    ...
    <>
      <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-10 py-10">
          ...
          {items.map((item, i) => (
            <div key={i} className="border border-emerald-400/12 bg-emerald-400/[0.02] px-6 py-5">
              <div className="flex items-start gap-4">
                <span className="text-xs font-mono text-white/15 shrink-0 mt-0.5">{String(i + 1).padStart(2, '0')}</span>
                <div className="flex-1 min-w-0">
                  <EditableField
                    value={item.label}
                    isEditing={isEditing}
                    onChange={v => updateItem(i, 'label', v)}
                    className="text-xs tracking-[0.25em] text-white/55 uppercase font-medium mb-1.5 block"
                  ...
                  <EditableField
                    ...
                    className="text-xs text-white/30 leading-relaxed block"
        ...
        <div className="mt-6">
          {isEditing ? (
            <div>
              <p className="text-xs text-white/30 mb-1">URL do Briefing Completo</p>
              <EditableField
            ...
            <a href={projeto.briefingUrl} target="_blank" rel="noopener noreferrer"
              ...
              <span className="text-xs tracking-[0.3em] text-white/40 uppercase">Ver Briefing Completo</span>
