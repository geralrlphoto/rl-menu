      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-10 py-10">
        ...
        <div className="grid grid-cols-3 gap-3 mb-10">
          <div className="border border-white/[0.07] bg-white/[0.02] px-4 py-5 text-center">
            <p className="text-[11px] tracking-[0.4em] text-white/25 uppercase mb-2">Incluídas</p>
            {isEditing ? (
        ...
        <div className="flex flex-col gap-3 mb-10">
          {Array.from({ length: total }).map((_, i) => (
            <div key={i}
              ...
              <p className={`text-[12px] tracking-[0.25em] uppercase ${i < usadas ? 'text-white/55' : 'text-white/20'}`}>
                Revisão {i + 1}
              </p>
              <span className={`ml-auto text-[11px] tracking-[0.35em] uppercase ${i < usadas ? 'text-emerald-400/70' : 'text-white/15'}`}>
        ...
        <div className="border border-white/[0.07] bg-white/[0.02] px-6 py-6">
          <p className="text-[11px] tracking-[0.4em] text-white/25 uppercase mb-3">Dar Feedback</p>
          <p className="text-[15px] font-light text-white/35 leading-relaxed mb-4">
            Quando receberes o link do vídeo para revisão, usa o botão abaixo para enviar os teus comentários.
          </p>
          <button disabled
            className="border border-white/10 bg-white/[0.03] px-5 py-2.5 text-[11px] tracking-[0.4em] text-white/20 uppercase cursor-not-allowed">
            Aguardar Vídeo
          </button>
        </div>
      </div>
