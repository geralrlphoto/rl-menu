      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 sm:px-10 py-10">
        ...
        <div className="mb-8 border border-white/[0.06] bg-white/[0.02] px-6 py-5 flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[12px] tracking-[0.5em] text-white/30 uppercase">Como funciona</p>
          </div>
          <p className="text-[16px] font-light leading-relaxed text-white/45">
            ...
            {[
              ...
              <div key={step.n} className="flex items-start gap-3">
                <span className="w-5 h-5 border border-white/15 flex items-center justify-center shrink-0
                                 text-[13px] tracking-widest text-white/25 mt-0.5">{step.n}</span>
                <p className="text-[15px] font-light text-white/35 leading-relaxed">{step.text}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <p className="text-[12px] tracking-[0.5em] text-white/60 group-hover:text-white/85 uppercase font-medium transition-colors">
                  Registar Pagamento
                </p>
                <p className="text-[15px] font-light text-white/25 group-hover:text-white/40 mt-0.5 transition-colors">
        ...
        <div className="grid grid-cols-3 gap-3 mb-10">
          <div className="border border-white/[0.07] bg-white/[0.02] px-4 py-5 text-center">
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-1">Total</p>
            <p className="text-[15px] tracking-[0.1em] text-white/65 font-light">{total.toLocaleString('pt-PT')} €</p>
            <p className="text-[12px] tracking-[0.3em] text-white/15 uppercase mt-1">Serviço</p>
          </div>
          <div className={`border px-4 py-5 text-center ${pago > 0 ? 'border-emerald-400/20 bg-emerald-400/[0.03]' : 'border-white/[0.07] bg-white/[0.02]'}`}>
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-1">Pago</p>
            <p className={`text-[15px] tracking-[0.1em] font-light ${pago > 0 ? 'text-emerald-400/80' : 'text-white/65'}`}>
              {pago.toLocaleString('pt-PT')} €
            </p>
            <p className="text-[12px] tracking-[0.3em] text-white/15 uppercase mt-1">{registos.length} registo{registos.length !== 1 ? 's' : ''}</p>
          </div>
          <div className={`border px-4 py-5 text-center ${restante > 0 ? 'border-white/[0.07] bg-white/[0.02]' : 'border-emerald-400/20 bg-emerald-400/[0.03]'}`}>
            <p className="text-[12px] tracking-[0.4em] text-white/25 uppercase mb-1">Restante</p>
            <p className={`text-[15px] tracking-[0.1em] font-light ${restante > 0 ? 'text-white/65' : 'text-emerald-400/80'}`}>
              {restante.toLocaleString('pt-PT')} €
            </p>
            <p className="text-[12px] tracking-[0.3em] text-white/15 uppercase mt-1">{restante === 0 ? 'Liquidado' : 'Em falta'}</p>
          </div>
        </div>
        ...
        {registos.length > 0 && (
          <div className="mb-10">
            <p className="text-[12px] tracking-[0.5em] text-white/20 uppercase mb-3">Histórico de Pagamentos</p>
                ...
                <div key={i} className="border border-emerald-400/15 bg-emerald-400/[0.02] px-5 py-3.5
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[14px] tracking-[0.2em] text-white/55">{r.data}</p>
                      {r.fase && (
                        <p className="text-[12px] tracking-[0.3em] text-white/25 uppercase mt-0.5 truncate">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[15px] tracking-[0.15em] text-emerald-400/80 font-light">
        ...
        <div className="mb-6">
          <p className="text-[12px] tracking-[0.5em] text-white/20 uppercase mb-3">Plano de Pagamento</p>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <EditableField
                        ...
                        className="text-[13px] tracking-[0.2em] text-white/60 uppercase font-medium block"
                      ...
                      <EditableDateField
                        ...
                        className="text-[12px] tracking-[0.2em] text-white/20 mt-0.5 block"
                    ...
                    <div className="text-right shrink-0">
                      <div className="flex items-center justify-end gap-1">
                        <EditableField
                          ...
                          type="number"
                          className="text-[15px] tracking-[0.1em] text-white/60 font-light block text-right"
                        />
                        {!isEditing && <span className="text-[15px] text-white/60 font-light">€</span>}
                      </div>
                      <div className="mt-1">
                        <EditableSelect
                          value={pag.estado}
                          ...
                          className={`text-[12px] tracking-[0.3em] uppercase ${cfg.color}`}
                        />
                      </div>
                      {isEditing && (
                        <button onClick={() => removePagamento(i)}
                          className="mt-2 text-[12px] tracking-[0.3em] text-red-400/50 hover:text-red-400/80 uppercase transition-colors">
                          Remover
                        </button>
                      )}
                    </div>
          ...
          {isEditing && (
            <button onClick={addPagamento}
              className="mt-3 w-full border border-dashed border-white/15 hover:border-white/30 bg-white/[0.01] hover:bg-white/[0.03] py-3
                         text-[12px] tracking-[0.4em] text-white/30 uppercase transition-colors">
              + Adicionar Linha
            </button>
          )}
        </div>
          ...
          <div className="relative w-full max-w-lg border border-white/[0.12] bg-[#04080f]"
            ...
            {submitted ? (
              <div className="px-8 py-12 flex flex-col items-center text-center gap-5">
                ...
                <div>
                  <p className="text-[12px] tracking-[0.5em] text-white/65 uppercase mb-2">Pagamento Registado</p>
                  <p className="text-[16px] font-light text-white/35 leading-relaxed">
                ...
                <button onClick={closeForm}
                  className="mt-2 border border-white/15 bg-white/[0.04] hover:bg-white/[0.08] px-6 py-2.5
                             text-[12px] tracking-[0.4em] text-white/45 hover:text-white/70 uppercase transition-all">
                  Fechar
                </button>
              </div>
            ) : (
              <>
                <div className="px-5 sm:px-8 pt-5 sm:pt-7 pb-2 flex items-center justify-between">
                  <div>
                    <p className="text-[9px] tracking-[0.5em] text-white/20 uppercase mb-1">RL PROD · {projeto.nome}</p>
                ...
                <div className="px-5 sm:px-8 py-5 sm:py-6 flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">Empresa</label>
                      <input
                        value={form.empresa}
                        onChange={e => setField('empresa', e.target.value)}
                        className="w-full bg-white/[0.05] border border-white/[0.10] text-white/70 text-[14px] px-3 py-2.5
                                   outline-none focus:border-white/30 placeholder-white/20 transition-colors"
                        placeholder="Nome da empresa"
                      />
                    </div>
                    <div>
                      <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">Referência</label>
                      <input
                        value={form.referencia}
                        readOnly
                        className="w-full bg-white/[0.02] border border-white/[0.06] text-white/35 text-[14px] px-3 py-2.5
                  ...
                  {/* Fase */}
                  <div>
                    <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-2">Fase do Pagamento</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['Adjudicação', 'Reforço', 'Final'].map(f => (
                        <button
                          ...
                          className={`py-2.5 border text-[12px] tracking-[0.3em] uppercase transition-all
                            ${form.fase === f
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">Valor (€)</label>
                      <input
                        type="number"
                        value={form.valor}
                    ...
                    <div>
                      <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">Método</label>
                      <select
                        value={form.metodo}
                  ...
                  {/* Data */}
                  <div>
                    <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">Data do Pagamento</label>
                    <input
                      type="date"
                      value={form.data}
                  ...
                  {/* Comprovativo */}
                  <div>
                    <label className="text-[12px] tracking-[0.4em] text-white/25 uppercase block mb-1.5">
                      Comprovativo <span className="text-white/15 normal-case tracking-normal text-[14px]">(foto ou PDF)</span>
                    </label>
                    <div
                    ...
                    >
                      {form.file ? (
                        <>
                          ...
                          <p className="text-[14px] text-white/55">{form.file.name}</p>
                          <p className="text-[14px] text-white/25">{(form.file.size / 1024).toFixed(0)} KB · clica para alterar</p>
                        </>
                      ) : (
                        <>
                          ...
                          <p className="text-[14px] text-white/30">Clica para fazer upload</p>
                          <p className="text-[14px] text-white/20">JPG, PNG, PDF</p>
                        </>
                      )}
                    </div>
                    <input
                  ...
                  {submitError && (
                    <p className="text-[13px] text-red-400/70 tracking-[0.2em]">{submitError}</p>
                  )}
                  ...
                  <button
                    ...
                    className="mt-1 w-full border border-white/25 hover:border-white/45 bg-white/[0.05] hover:bg-white/[0.10]
                               px-6 py-3.5 text-[12px] tracking-[0.5em] text-white/55 hover:text-white/85 uppercase
