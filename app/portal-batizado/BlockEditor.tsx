      ...
      for (const col of (b.children ?? []) as Block[]) {
        for (const child of (col.children ?? []) as Block[]) {
          if (child.type === 'image') {
            ...
            // Non-text, non-image (table, embed, etc.) → non-editable placeholder
            items.push({
              key: child.id,
              id: child.id,
              type: child.type,
...
export default function BlockEditor({
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (const it of items) {
        ...
        if (it.extraIds !== undefined) {
          if (!it.isDeleted) {
            ...
            // Extra block IDs are now merged into the first; remove stale overrides
      ...
      // Spread full settings so unrelated fields (subpageOwnHeader, etc.) are preserved
