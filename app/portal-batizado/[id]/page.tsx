        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          {album?.id && !isAprovado && (
            <div className="grid grid-cols-2 gap-3 pt-1">
              ...
              <a href={`/portal-batizado/album-alteracao?ref=${encodeURIComponent(portalRef)}`}
                  ...
                  {(['NOIVO', 'NOIVA', 'CERIMÓNIA', 'QUINTA'] as const).map(section => (
                    <div key={section} className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
                      ...
                      <input
                        ...
                        placeholder="https://... ou /portal-batizado/..."
                        className="w-full bg-white/[0.04] border border-white/10 rounded-lg px-3 py-2 text-sm text-white/80 outline-none focus:border-gold/40 transition-colors placeholder:text-white/20"
                      />
                    </div>
                  ))}
              ...
              : (
                <>
                  ...
                  {(() => {
                    ...
                    if (isContratoPage) {
                      ...
                      return (
                        <>
                          ...
                          <NotionBlocks blocks={blocks.filter(b => b.type !== 'image')} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                        </>
                      )
                    }
                    if (isCronogramaPage) {
                      const backUrlCron = fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined
                      return (
                    ...
                    if (isSatisfacaoPage) {
                      const backUrlSat = fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined
                    ...
                    if (isBriefingChildPage) {
                      ...
                      return (
                        <>
                          ...
                          <NotionBlocks blocks={blocks} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                        </>
                      )
                    }
                    if (isGuiaPage) {
                      ...
                      const SECTION_META: Record<string, { icon: string; label: string; url: string | undefined }> = {
                        ...
                        'PAGAMENTOS/REGISTO':        { icon: '💳', label: 'Registar Pagamento',   url: `/registar-pagamento?ref=${encodeURIComponent(portalSettingsObj?.referencia ?? refParam ?? portalRef ?? '')}&noivos=${encodeURIComponent([portalSettingsObj?.noiva ?? portalSettingsObj?.mae, portalSettingsObj?.noivo ?? portalSettingsObj?.pai].filter(Boolean).join(' & '))}` },
                      }
                      ...
                      if (colListIdx === -1) return <NotionBlocks blocks={blocks} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                    ...
                    if (!isPaymentsPage) {
                      ...
                      while (i < blocks.length) {
                        ...
                        if (cardsInBlock.length > 0) {
                          ...
                          renderedSections.push(
                            <NotionBlocks key={`block-${i}`} blocks={[b]} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                          )
                        }
                        i++
                      }
                    ...
                    return (
                      <>
                        ...
                        {numerarioIdx !== -1 && (
                          <div className="my-5">
                            <a href={`/registar-pagamento?ref=${encodeURIComponent(portalSettingsObj?.referencia ?? refParam ?? portalRef ?? '')}&noivos=${encodeURIComponent([portalSettingsObj?.noiva ?? portalSettingsObj?.mae, portalSettingsObj?.noivo ?? portalSettingsObj?.pai].filter(Boolean).join(' & '))}`} target="_blank" rel="noopener noreferrer"
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              <NotionBlocks blocks={beforeValor} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                              ...
                              <PaymentPhasesSection
                                ...
                                refreshing={pagRefreshing}
                              />
                              <NotionBlocks blocks={afterValor} hiddenNav={settings.hiddenNav} backUrl={fromId ? `/portal-batizado/${fromId}?title=${encodeURIComponent(fromTitle ?? '')}${refParam ? `&portalRef=${encodeURIComponent(refParam)}` : ''}` : refParam ? `/portal-batizado/ref/${encodeURIComponent(refParam)}` : undefined} />
                            </>
                          )
                        })()}
                      </>
