  ...
  return (
    <div className="rl-portal min-h-screen">
      ...
      <header id="rlp-bar" className="rlp-bar" style={{ top: isAdmin ? 34 : 0 }}>
        <a href={`/b/${token}`} className="rlp-mono" aria-label="RL Photo · Video">
      ...
      <section id="sec-reuniao" className="relative flex items-end overflow-hidden" style={{ minHeight: '92svh', paddingTop: isAdmin ? 60 : 0 }}>
        ...
        <div className="rlp-wrap relative z-10 w-full" style={{ paddingBottom: 'clamp(48px,8vh,110px)' }}>
          ...
          <FadeIn delay={380} className="mt-6 flex flex-col gap-3" style={{ maxWidth: '52ch' }}>
            {(crmNome || evento.nome) && <p className="rlp-ed" style={{ fontSize: 'clamp(20px,2.4vw,30px)', lineHeight: 1.2 }}><em>{crmNome || evento.nome}</em></p>}
            {dataFmt && <p style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--tx-mid)' }}>{dataFmt}{evento.hora ? ` · ${evento.hora}` : ''}{evento.local ? ` · ${evento.local}` : ''}</p>}
          </FadeIn>
        </div>
      </section>
      ...
      <section className="rlp-sec" style={{ paddingLeft: 'var(--pad)', paddingRight: 'var(--pad)', paddingBottom: 0 }}>
        <FadeIn className="mx-auto w-full" style={{ maxWidth: 420 }}>
          <div className="rlp-card w-full">
            <div style={{ padding: '22px 26px', borderBottom: '1px solid var(--line-soft)' }} className="flex items-center justify-between">
              <span className="rlp-eyebrow">Detalhes do Batizado</span>
              {isAdmin && (
            ...
            {editingEvento && isAdmin ? (
              <div style={{ padding: '18px 26px 22px' }} className="flex flex-col gap-3">
              ...
              <div style={{ padding: '8px 26px 20px' }}>
                {evento.nome && <div className="rlp-drow"><span className="k">Nome</span><span className="v">{evento.nome}</span></div>}
                {evento.data && <div className="rlp-drow"><span className="k">Data</span><span className="v">{dataFmt}</span></div>}
                {evento.hora && <div className="rlp-drow"><span className="k">Hora</span><span className="v">{evento.hora}</span></div>}
                {evento.local && <div className="rlp-drow"><span className="k">Local</span><span className="v" style={{ textAlign: 'right', maxWidth: 200 }}>{evento.local}</span></div>}
              </div>
            )}
          </div>
        </FadeIn>
      ...
      {targetDate && (
        <section id="sec-countdown" className="border-y" style={{ borderColor: 'var(--line-soft)', background: 'var(--ink-2)' }}>
          <div className="rlp-wrap flex flex-col items-center gap-8" style={{ paddingTop: 'clamp(46px,7vh,88px)', paddingBottom: 'clamp(46px,7vh,88px)' }}>
            <FadeIn><span className="rlp-eyebrow c">Contagem Regressiva</span></FadeIn>
      ...
      <section className="rlp-sec" style={{ paddingLeft: 'var(--pad)', paddingRight: 'var(--pad)' }}>
        ...
        <div className="rlp-card w-full flex-1">
          ...
          <div style={{ padding: '8px 26px 20px' }}>
            <div className="rlp-drow"><span className="k">Data</span><span className="v">{reuniaoData || '—'}</span></div>
            <div className="rlp-drow"><span className="k">Hora</span><span className="v">{reuniaoHora || '—'}</span></div>
            <div className="rlp-drow"><span className="k">Modo</span><span className="v">{reuniao.tipo || 'Presencial'}</span></div>
          </div>
          {reuniao.link && (
            <div style={{ padding: '0 26px 18px' }}>
              <a href={reuniao.link} target="_blank" rel="noopener noreferrer" className="rlp-btn ghost full">
          ...
          {targetDate && (() => {
            ...
            const start = fmt(reuniao.data, reuniaoHora)
            const end   = fmt(reuniao.data, String(parseInt(reuniaoHora.split(':')[0]) + 1).padStart(2,'0') + ':' + reuniaoHora.split(':')[1])
            const title = encodeURIComponent('Reunião RL Photo · Video')
            const loc   = encodeURIComponent(reuniao.link || (reuniao.tipo === 'Videochamada' ? 'Videochamada' : 'Presencial'))
          ...
          {(() => {
            if (propostaResposta === 'confirmada') {
              ...
              <div className="rlp-card w-full text-center flex-1" style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 22 }}>
                ...
                <div>
                  ...
                  <p className="rlp-ed" style={{ fontSize: 18, marginTop: 6 }}>para o batizado</p>
                </div>
                ...
                <a href={contratoUrl} className="rlp-btn full">
            ...
            if (propostaResposta === 'rejeitada') return (
              <div className="rlp-card w-full text-center flex-1" style={{ padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14 }}>
                ...
                <p className="rlp-lede" style={{ fontSize: 14, maxWidth: '42ch' }}>
                  Foi um prazer conhecer-vos e esperamos poder trabalhar juntos no futuro. Desejamos-vos o melhor para este dia especial.
                </p>
              </div>
            )
            ...
            return (
              <div className="flex flex-col gap-3 w-full flex-1">
                ...
                {/* Confirmar */}
                <button onClick={() => handleProposta('confirmar')} disabled={submittingProposta || isAdmin} className="rlp-btn full">
                  <span className="fill" /><span className="dot" />{submittingProposta ? 'A processar…' : 'Confirmar Proposta'}
                </button>
                {/* Rejeitar — discreto */}
                <button onClick={() => handleProposta('rejeitar')} disabled={submittingProposta || isAdmin} className="rlp-btn danger full">
          ...
          <div className="rlp-card" style={{ padding: 'clamp(32px,5vw,64px)' }}>
            <div className="flex flex-col md:flex-row md:items-center gap-10 relative z-10">
              ...
              <div className="flex flex-col items-stretch gap-3 md:w-72 flex-shrink-0">
                <a href={`/b/${token}/proposta`} className="rlp-btn full">
