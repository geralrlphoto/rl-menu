...
// ─────────────────────────────────────────────────────────────────────────────
export default function BatizadoPropostaClient({ token, isAdmin }: { token: string; isAdmin: boolean }) {
  ...
  useEffect(() => {
    fetch(`/api/batizado/view?token=${token}`)
      ...
      .then(data => {
        ...
        const stored = typeof window !== 'undefined' ? localStorage.getItem(`batizado_proposta_${token}`) : null
  ...
  const handleUnlock = () => {
    ...
    if (!pw || pwInput === pw) {
      if (typeof window !== 'undefined') localStorage.setItem(`batizado_proposta_${token}`, pwInput)
      setLocked(false)
    } else {
  ...
  const handlePublish = async () => {
    ...
    await fetch('/api/batizado/sync-template', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
    setPublished(true); setTimeout(() => setPublished(false), 3000); setPublishing(false)
  }

  function setPage(k: keyof BatizadoContent['propostaPage'], v: any) {
    setContent(c => ({ ...c, propostaPage: { ...c.propostaPage, [k]: v } }))
  }
  function setTypo(k: keyof BatizadoContent['propostaPage']['typography'], v: string) {
  ...
  function setPkg(i: number, k: 'title' | 'description' | 'price', v: string) {
    setContent(c => { const packages = [...c.propostaPage.packages]; packages[i] = { ...packages[i], [k]: v }; return { ...c, propostaPage: { ...c.propostaPage, packages } } })
  }
  function setAboutSlide(k: keyof BatizadoContent['propostaPage']['about'], v: string) {
    setContent(c => ({ ...c, propostaPage: { ...c.propostaPage, about: { ...c.propostaPage.about, [k]: v } } }))
  }
  function setRelive(k: keyof BatizadoContent['propostaPage']['relive'], v: string) {
      ...
      <div className="w-full max-w-sm">
        <div className="relative" style={{ border: '0.5px solid rgba(201,168,76,0.3)', background: 'linear-gradient(135deg, #1c1408 0%, #0f0c07 50%, #1c1408 100%)' }}>
          ...
          <div className="px-10 py-12 text-center flex flex-col items-center gap-6">
            ...
            <a href={`/b/${token}`} className="text-[10px] tracking-widest text-white/20 hover:text-white/40 transition-colors uppercase">← Voltar</a>
          </div>
        </div>
      </div>
    </main>
  ...
  const nome = content.evento?.nome || ''
    ...
    switch (id) {
          ...
          <div className="flex flex-row items-center gap-10 sm:gap-14 w-full max-w-5xl justify-center">
            ...
            <div className="relative flex-shrink-0" style={{ width: 'clamp(180px,24vw,300px)', height: 'clamp(260px,36vw,430px)' }}>
              ...
              {pp.about?.videoUrl && toEmbed(pp.about.videoUrl)
                ? <iframe src={toEmbed(pp.about.videoUrl)} className="w-full h-full" style={{ border: 'none' }} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen />
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-3 w-full max-w-5xl">
            ...
            <div className="flex-shrink-0 self-stretch" style={{ width: 'clamp(200px,30vw,360px)' }}>
              {pp.grandeDia?.imageUrl
                ? <img src={pp.grandeDia.imageUrl} alt="" className="w-full h-full object-cover" style={{ borderRadius: '8px' }} />
      ...
      case 'pkg-2': {
        ...
        const toggleExtraSlide = (nome: string) =>
          setExtrasSelected(prev => { const cur = prev[idx] || []; const has = cur.includes(nome); return { ...prev, [idx]: has ? cur.filter(n => n !== nome) : [...cur, nome] } })
          ...
          <div className="flex items-center justify-center h-full w-full px-4 sm:px-8">
            <div className="relative w-full" style={{ maxWidth: '980px' }}>
              ...
              <div className="rl-stub flex flex-col"
                ...
                {escolhida === idx ? (
                  <a href={`/b/${token}`} className="btn" style={{ width: '100%' }}>
                    <span className="fill" />
                    Escolhida
                  </a>
                ) : (
        ...
        <div className="flex items-center justify-center h-full w-full px-8 sm:px-16 py-8">
          <div className="w-full max-w-5xl flex flex-col gap-6">
            ...
            <ul className="flex flex-col gap-2.5" style={{ fontSize: '20px', color: typo.bodyColor, opacity: 0.75, lineHeight: 1.65 }}>
              ...
              <li className="flex gap-2"><span style={{ color: typo.accentColor, flexShrink: 0 }}>–</span><span>Entrega até um prazo estimado de <strong style={{ color: typo.accentColor, fontWeight: 500 }}>90 dias úteis</strong>;</span></li>
              ...
              <li className="flex gap-2" style={{ opacity: 0.45, fontSize: '14px', fontStyle: 'italic' }}><span style={{ color: typo.accentColor, flexShrink: 0 }}>*</span><span>Deslocação fora da zona de Setúbal/Almada/Montijo poderá ter valor extra.</span></li>
            </ul>
            <div className="h-px" style={{ background: `${typo.accentColor}25` }} />
            <a href={`/b/${token}`}
      ...
      case 'contact': return (
        <div className="relative flex items-center justify-center h-full w-full overflow-hidden">
          ...
          {/* Conteúdo — layout duas colunas */}
          <div className="relative z-10 flex flex-row items-center w-full h-full px-16 sm:px-20 gap-12">
    ...
    <div className="nlead relative w-full overflow-hidden" style={{ height: '100dvh', background: '#0b0a08' }}>
      ...
      {isAdmin && (
        <div className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-2 bg-black/70 backdrop-blur-sm border-b border-white/5">
          <a href={`/b/${token}`} className="text-[10px] tracking-widest text-white/25 hover:text-white/50 transition-colors uppercase">‹ Página</a>
          <span className="text-[10px] tracking-widest text-white/15 uppercase">Admin · Proposta Batizado</span>
          ...
          <div className={`fixed top-0 right-0 h-full z-50 flex flex-col transition-transform duration-300 ${editorOpen ? 'translate-x-0' : 'translate-x-full'}`}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
              {editorTab === 'tipografia' && <>
                ...
                <Field label="Tipo de letra"><FontPicker value={typo.titleFont} onChange={v => setTypo('titleFont', v)} /></Field>
                <Field label="Tamanho"><SizePicker value={typo.titleSize} onChange={v => setTypo('titleSize', v)} /></Field>
                <Field label="Cor"><ColorPicker value={typo.titleColor} onChange={v => setTypo('titleColor', v)} /></Field>
                <div className="h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
                <p className="text-[9px] tracking-[0.3em] text-white/20 uppercase">Corpo / Introdução</p>
                <Field label="Tipo de letra"><FontPicker value={typo.bodyFont} onChange={v => setTypo('bodyFont', v)} /></Field>
                <Field label="Cor"><ColorPicker value={typo.bodyColor} onChange={v => setTypo('bodyColor', v)} /></Field>
                <div className="h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
                <p className="text-[9px] tracking-[0.3em] text-white/20 uppercase">Nome dos pacotes</p>
                <Field label="Tipo de letra"><FontPicker value={typo.pkgTitleFont} onChange={v => setTypo('pkgTitleFont', v)} /></Field>
                <Field label="Cor"><ColorPicker value={typo.pkgTitleColor} onChange={v => setTypo('pkgTitleColor', v)} /></Field>
                <div className="h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
                <p className="text-[9px] tracking-[0.3em] text-white/20 uppercase">Cor de destaque</p>
                <Field label="Dourado / Accent"><ColorPicker value={typo.accentColor} onChange={v => setTypo('accentColor', v)} /></Field>
              </>}
              {editorTab === 'texto' && <>
                <Field label="Subtítulo (capa)"><TInput value={pp.subtitle} onChange={v => setPage('subtitle', v)} /></Field>
                <Field label="Texto de introdução"><TInput value={pp.intro} onChange={v => setPage('intro', v)} multiline /></Field>
                <div className="h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
                ...
                <Field label="Foto (slide 2)">
                  ...
                  {pp.about?.photo && (
                    <div className="relative mt-1 rounded-lg overflow-hidden" style={{ height: '90px' }}>
                      ...
                      <button onClick={() => setAboutSlide('photo', '')}
                        className="absolute top-1 right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px]"
                        style={{ background: 'rgba(0,0,0,0.7)', color: 'rgba(255,255,255,0.5)' }}>✕</button>
                    </div>
                  )}
                </Field>
                <Field label="Título"><TInput value={pp.about?.title || ''} onChange={v => setAboutSlide('title', v)} placeholder="Sobre Nós" /></Field>
                <Field label="Posição do título">
                  <div className="grid grid-cols-3 gap-1">
                    {([['top-left','↖'],['top-center','↑'],['top-right','↗'],['mid-left','←'],['mid-center','·'],['mid-right','→'],['bot-left','↙'],['bot-center','↓'],['bot-right','↘']] as [string,string][]).map(([pos, arrow]) => (
                      <button key={pos} onClick={() => setAboutSlide('titlePos', pos)}
                ...
                <Field label="Parágrafo 1"><TInput value={pp.grandeDia?.p1 || ''} onChange={v => setGrandeDia('p1', v)} multiline /></Field>
                <Field label="Parágrafo 2"><TInput value={pp.grandeDia?.p2 || ''} onChange={v => setGrandeDia('p2', v)} multiline /></Field>
                <Field label="Parágrafo 3"><TInput value={pp.grandeDia?.p3 || ''} onChange={v => setGrandeDia('p3', v)} multiline /></Field>
                <Field label="Nota de rodapé"><TInput value={pp.grandeDia?.note || ''} onChange={v => setGrandeDia('note', v)} multiline /></Field>
                <Field label="Imagem">
                ...
                <Field label="Texto final (CTA)"><TInput value={pp.ctaText} onChange={v => setPage('ctaText', v)} /></Field>
              </>}
            </div>
