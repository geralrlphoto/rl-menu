...
const IMG_URL     = 'https://rl-menu-lake.vercel.app/card-edicao-fotos.png'
...
export async function POST(req: NextRequest) {
  ...
  const subject = nome_noivos
    ? `Nova edição de fotos · ${nome_noivos}`
    : `Nova edição de fotos atribuída a ti`
...
<body style="margin:0;padding:0;background:#0d0901;">
    ...
    <tr>
      <td align="center">
        <a href="${portalUrl}" style="display:block;text-decoration:none;">
          <img src="${IMG_URL}"
            width="320" alt="Nova edição de fotos"
  ...
  if (!res.ok) {
    console.error('[send-edicao-fotos-email]', data)
