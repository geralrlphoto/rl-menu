...
import { CAMPOS_BRIEFING, emFaltaBriefing, limparBriefing, valorBriefing } from '@/lib/briefing'
...
export async function POST(req: NextRequest) {
  ...
  if (!ev || ev.batizado) return NextResponse.json({ error: 'Link inválido' }, { status: 404 })
  ...
  const r = limparBriefing(respostas)
  const falta = emFaltaBriefing(r)
  ...
  const linhas = CAMPOS_BRIEFING.map(c => {
...
<tr><td style="padding:28px 32px 6px;color:#C9A84C;font-size:11px;letter-spacing:4px;text-transform:uppercase">Briefing pré-casamento ${primeira ? 'recebido' : 'atualizado'}</td></tr>
<tr><td style="padding:2px 32px 4px;color:#fff;font-size:26px;font-family:Georgia,serif">${esc(ev.nome || ev.cliente || '')}</td></tr>
<tr><td style="padding:0 32px 14px;color:rgba(255,255,255,.5);font-size:13px">Casamento: ${ev.data_evento ? esc(fmtDataLonga(ev.data_evento)) : '—'}</td></tr>
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Briefing ${primeira ? 'recebido' : 'atualizado'}: ${ev.nome || ev.cliente}`, html,
      }),
    })
