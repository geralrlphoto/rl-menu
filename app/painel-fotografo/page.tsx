...
export default function PainelEditor() {
  ...
  function loadFromStorage() {
    try {
      const raw = localStorage.getItem('painel-fotografo-user-projects')
      ...
      try {
        const patchesJson = localStorage.getItem('painel-fotografo-project-patches')
      ...
      // Ler unseen ids
      const unseenJson = localStorage.getItem('painel-fotografo-unseen-projects')
  ...
  const calendarMarks = useMemo(() => {
    ...
    try {
      const raw = typeof window !== 'undefined' ? localStorage.getItem('painel-fotografo-user-projects') : null
      const userProjects: any[] = raw ? JSON.parse(raw) : []
    ...
    try {
      ...
      const userTasksRaw = w ? localStorage.getItem('painel-fotografo-user-tasks') : null
      const userTasks: any[] = userTasksRaw ? JSON.parse(userTasksRaw) : []
      const delRaw = w ? localStorage.getItem('painel-fotografo-deleted-tasks') : null
      const deleted = new Set<string>(delRaw ? JSON.parse(delRaw) : [])
      const userProjRaw = w ? localStorage.getItem('painel-fotografo-user-projects') : null
      const userProj: any[] = userProjRaw ? JSON.parse(userProjRaw) : []
      const patchesRaw = w ? localStorage.getItem('painel-fotografo-project-patches') : null
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const raw = localStorage.getItem('painel-fotografo-notified-overdue')
        ...
        for (const proj of pending) {
          ...
          try {
            ...
            if (res.ok && data?.ok) {
              ...
              localStorage.setItem('painel-fotografo-notified-overdue', JSON.stringify([...notified]))
            }
          } catch {}
        }
      } catch {}
  ...
  const compromissos = useMemo(() => {
    ...
    try {
      const raw = localStorage.getItem('painel-fotografo-user-tasks')
      const userTasks: any[] = raw ? JSON.parse(raw) : []
      const delRaw = localStorage.getItem('painel-fotografo-deleted-tasks')
      const deleted = new Set<string>(delRaw ? JSON.parse(delRaw) : [])
      const userProjRaw = localStorage.getItem('painel-fotografo-user-projects')
      const userProj: any[] = userProjRaw ? JSON.parse(userProjRaw) : []
      const patchesRaw = localStorage.getItem('painel-fotografo-project-patches')
      const patches: Record<string, any> = patchesRaw ? JSON.parse(patchesRaw) : {}
      const deletedProjIds = new Set<string>()
  ...
  // ── Recebimentos Total Anual (mesmo cálculo do /painel-fotografo/pagamentos) ──
  ...
  const totalAnualEuros = useMemo(() => {
    ...
    try {
      ...
      const ovRaw = localStorage.getItem('painel-fotografo-payment-overrides')
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('painel-fotografo-unseen-tasks')
  ...
  function markTaskSeen(id: string) {
    setUnseenTaskIds(prev => {
      ...
      try { localStorage.setItem('painel-fotografo-unseen-tasks', JSON.stringify([...next])) } catch {}
      return next
    })
  }
  ...
  function markProjectSeen(id: string) {
    setUnseenIds(prev => {
      if (!prev.has(id)) return prev
      const next = new Set(prev); next.delete(id)
      try { localStorage.setItem('painel-fotografo-unseen-projects', JSON.stringify([...next])) } catch {}
      return next
    })
  }
  ...
  return (
    <div className="min-h-screen text-white relative" style={{ background: '#0B0B0B' }}>
      ...
      {/* ── SIDEBAR ───────────────────────────────────────────────────────── */}
      <aside
        className="hidden md:flex fixed top-0 left-0 bottom-0 w-[220px] z-30 flex-col"
        style={{
      ...
      {/* ── MAIN ──────────────────────────────────────────────────────────── */}
      <main className="relative z-10 md:pl-[220px]">
        <div className="px-6 sm:px-8 py-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-1 flex flex-col gap-5">
              ...
              <div className="rounded-2xl border border-white/[0.08] p-5"
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/painel-fotografo/novos-projetos" className="text-[11px] tracking-widest uppercase text-gold/70 hover:text-gold transition-colors">Ver todos →</Link>
                </div>
                <div className="space-y-3">
                  {novosProjetos.map(p => {
                    ...
                    return (
                      <Link key={p.id}
                        href={`/painel-fotografo/novos-projetos?open=${p.id}`}
              ...
              <div className="rounded-2xl border border-white/[0.08] p-5"
                ...
                <div className="grid grid-cols-2 gap-3">
                  {finalizadosProjetos.map(p => (
                    <Link key={p.id} href={`/painel-fotografo/novos-projetos?open=${p.id}`} className="group cursor-pointer">
            ...
            <div className="lg:col-span-1 flex flex-col gap-5">
              ...
              {projetosAtrasados.length > 0 && (
                <div className="alert-glow rounded-2xl border border-red-500/40 p-5 mb-5"
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Link href="/painel-fotografo/novos-projetos"
                  ...
                  <div className="space-y-2">
                    {projetosAtrasados.slice(0, 5).map(p => (
                      <Link key={p.id} href={`/painel-fotografo/novos-projetos?open=${p.id}`}
              ...
              {/* Próximos compromissos — tarefas reais criadas em /painel-fotografo/tarefas */}
              <div className="rounded-2xl border border-white/[0.08] p-5"
                style={{ background: 'linear-gradient(180deg, rgba(20,15,8,0.4), rgba(11,11,11,0.7))', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.5)' }}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-[15px] font-semibold text-white">Próximos compromissos</h3>
                  <Link href="/painel-fotografo/tarefas" className="text-[11px] tracking-widest uppercase text-gold/70 hover:text-gold transition-colors">Ver todas →</Link>
                </div>
                <div className="space-y-3">
                  {compromissos.length === 0 ? (
                    <p className="text-[12px] text-white/30 italic py-2">Sem compromissos próximos. <Link href="/painel-fotografo/tarefas" className="text-gold/70 hover:text-gold underline">Criar tarefa →</Link></p>
            ...
            <div className="lg:col-span-1 flex flex-col gap-5">
              ...
              <div className="rounded-2xl border border-white/[0.08] p-5"
                ...
                <div className="space-y-4">
                  {tarefasWidget.length === 0 ? (
                    <p className="text-[12px] text-white/30 italic py-2">Sem tarefas. <Link href="/painel-fotografo/tarefas" className="text-gold/70 hover:text-gold underline">Criar tarefa →</Link></p>
                  ) : tarefasWidget.map(t => {
                    const isUnseen = unseenTaskIds.has(t.id) && !t.done
                    return (
                      <Link key={t.id} href={`/painel-fotografo/tarefas`}
              ...
              <div className="rounded-2xl border border-white/[0.08] p-5"
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/painel-fotografo/novos-projetos" className="text-[11px] tracking-widest uppercase text-gold/70 hover:text-gold transition-colors">Ver →</Link>
                </div>
          ...
          <div className="mt-10 mb-4 flex items-center justify-between">
            <p className="text-[10px] tracking-[0.4em] uppercase text-white/20">RL Photo.Video · Painel Fotógrafo</p>
