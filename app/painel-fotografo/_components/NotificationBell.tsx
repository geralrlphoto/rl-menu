...
//  Usado em todas as páginas de /painel-fotografo.
// ──────────────────────────────────────────────────────────────────────
...
export function NotificationBell() {
  ...
  const notifications = useMemo<Notif[]>(() => {
    ...
    try {
      ...
      const unseenProjRaw = localStorage.getItem('painel-fotografo-unseen-projects')
      ...
      unseenProjIds.forEach(id => {
        ...
        items.push({
          ...
          href: `/painel-fotografo/novos-projetos?open=${id}`,
        })
      })

      // 2) Tarefas não vistas
      const unseenTaskRaw = localStorage.getItem('painel-fotografo-unseen-tasks')
      ...
      userTasks
        ...
        .forEach(t => {
          ...
          items.push({
            ...
            href: '/painel-fotografo/tarefas',
          })
        })

      // 3) Mensagens não lidas (varre todas as chaves painel-fotografo-mensagens-{projectId})
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (!key || !key.startsWith('painel-fotografo-mensagens-')) continue
        if (key.includes('-seen-')) continue   // skip seen counters
        const projectId = key.replace('painel-fotografo-mensagens-', '')
        try {
          const arr: any[] = JSON.parse(localStorage.getItem(key) || '[]')
          if (arr.length === 0) continue
          const seenAdmin = Number(localStorage.getItem(`painel-fotografo-mensagens-seen-admin-${projectId}`) ?? 0)
          const seenFree  = Number(localStorage.getItem(`painel-fotografo-mensagens-seen-freelancer-${projectId}`) ?? 0)
          ...
          items.push({
            ...
            href: `/painel-fotografo/novos-projetos?open=${projectId}&chat=1`,
          })
        } catch {}
      }
    } catch {}
  ...
  function clearOne(notifId: string) {
    try {
      if (notifId.startsWith('proj-')) {
        ...
        const raw = localStorage.getItem('painel-fotografo-unseen-projects')
        const arr: string[] = raw ? JSON.parse(raw) : []
        localStorage.setItem('painel-fotografo-unseen-projects', JSON.stringify(arr.filter(x => x !== id)))
      } else if (notifId.startsWith('task-')) {
        const id = notifId.replace('task-', '')
        const raw = localStorage.getItem('painel-fotografo-unseen-tasks')
        const arr: string[] = raw ? JSON.parse(raw) : []
        localStorage.setItem('painel-fotografo-unseen-tasks', JSON.stringify(arr.filter(x => x !== id)))
      } else if (notifId.startsWith('msg-')) {
        // Marca msgs do projeto como vistas pelos dois lados
        const projectId = notifId.replace('msg-', '')
        const arr: any[] = JSON.parse(localStorage.getItem(`painel-fotografo-mensagens-${projectId}`) || '[]')
        localStorage.setItem(`painel-fotografo-mensagens-seen-admin-${projectId}`, String(arr.length))
        localStorage.setItem(`painel-fotografo-mensagens-seen-freelancer-${projectId}`, String(arr.length))
      }
      setTick(t => t + 1)
    } catch {}
  }
  ...
  function clearAll() {
    try {
      localStorage.setItem('painel-fotografo-unseen-projects', JSON.stringify([]))
      localStorage.setItem('painel-fotografo-unseen-tasks', JSON.stringify([]))
      // Marca todas as conversas como totalmente lidas
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i)
        if (!key || !key.startsWith('painel-fotografo-mensagens-') || key.includes('-seen-')) continue
        const projectId = key.replace('painel-fotografo-mensagens-', '')
        const arr: any[] = JSON.parse(localStorage.getItem(key) || '[]')
        localStorage.setItem(`painel-fotografo-mensagens-seen-admin-${projectId}`, String(arr.length))
        localStorage.setItem(`painel-fotografo-mensagens-seen-freelancer-${projectId}`, String(arr.length))
    ...
    <div
      ...
      {count > 0 && (
        <div className="px-4 py-2.5 border-t border-white/[0.06] bg-black/30 flex items-center justify-between">
          ...
          <Link href="/painel-fotografo/tarefas" onClick={() => setOpen(false)}
