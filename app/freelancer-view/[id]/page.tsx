        ...
        return (
          <div className="flex flex-col gap-5">
            ...
            <div className="relative overflow-hidden rounded-2xl border border-white/[0.08]"
              style={{ background: 'linear-gradient(90deg, rgba(8,6,4,0.95) 0%, rgba(14,11,7,0.85) 45%, rgba(14,11,7,0.4) 70%, rgba(14,11,7,0.1) 100%)' }}>
              {/* Foto à direita */}
              <div className="absolute inset-y-0 right-0 w-full lg:w-[55%] z-0">
                <img
                  src="https://images.pexels.com/photos/1024993/pexels-photo-1024993.jpeg?auto=compress&cs=tinysrgb&w=1400"
                  alt="Casamento"
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-6 p-6 sm:p-8">
                ...
                <div className="lg:col-span-3 flex flex-col gap-5">
                  <div>
                    <h1 className="text-3xl sm:text-4xl font-semibold text-white tracking-tight">
                      Bem-vindo de volta, <span className="text-white">{primeiroNome}</span>! <span className="inline-block">👋</span>
                    </h1>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {[
                        { icon: '◫', value: totalCasamentos, label: 'Casamentos',     color: 'text-gold',     bg: 'bg-gold/10 border-gold/25' },
                        { icon: '✎', value: totalEmEdicao,    label: 'Em edição',     color: 'text-purple-300', bg: 'bg-purple-500/10 border-purple-500/25' },
                        { icon: '✓', value: totalConcluidos,  label: 'Concluídos',    color: 'text-emerald-300', bg: 'bg-emerald-500/10 border-emerald-500/25' },
                        { icon: '◷', value: totalAguardando,  label: 'Aguardando',    color: 'text-amber-300',  bg: 'bg-amber-500/10 border-amber-500/25' },
                      ].map((s, i) => (
                        <div key={i} className="bg-black/30 border border-white/[0.06] rounded-xl px-3 py-2.5 flex items-center gap-2.5">
                          <span className={`w-8 h-8 rounded-lg border flex items-center justify-center ${s.bg} ${s.color} text-base`}>{s.icon}</span>
                          <div className="min-w-0">
                            <p className="text-2xl font-bold text-white leading-none">{s.value}</p>
                            <p className="text-[14px] text-white/45 leading-tight mt-0.5">{s.label}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="lg:col-span-2 hidden lg:flex items-center justify-end">
                  <div className="text-right max-w-[260px]">
                    <p className="text-gold/60 text-4xl font-serif leading-none mb-2">"</p>
                    <p className="text-[15px] text-white/75 italic leading-relaxed font-light">
                      Transformamos momentos em histórias que duram para sempre.
                    </p>
            ...
            {/* ── Resto do dashboard (grid 2/3 + 1/3) ────────────────── */}
