...
const TIPO_CLS: Record<string, string> = {
  ...
  'PRODUÇÃO':   'bg-white/10 border-white/20 text-white/60',
}
...
export default function PortalClientePage() {
  ...
  const statuses = ['Todos', ...Array.from(new Set(portais.map(p => p.status).filter(Boolean)))]
  const tipos = ['Todos', ...Array.from(new Set(portais.map(p => p.tipo_evento).filter(Boolean)))]
  ...
  return (
    <main className="min-h-screen px-3 sm:px-6 py-6 sm:py-10 max-w-[1200px] mx-auto">
      ...
      <header className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-light tracking-widest text-gold uppercase">Portal do Cliente</h1>
      ...
      {!loading && !error && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            <div key={s.label} className="bg-white/[0.03] border border-white/[0.07] rounded-2xl px-4 py-4 flex flex-col gap-1">
      ...
      {/* Loading / Error */}
      {loading && (
        <div className="text-center py-24 text-white/20 text-xs tracking-widest uppercase">A carregar portais...</div>
      )}
      {error && (
        ...
        <>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {filtered.map(p => (
                <div key={p.id} className="bg-white/[0.02] border border-white/[0.07] hover:border-gold/25 rounded-2xl px-4 sm:px-6 py-4 sm:py-5 transition-all">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        {/* Status dot */}
                        {p.status && (
                          <span className={`w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[p.status] ?? 'bg-white/30'}`} />
                        )}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-1.5 text-xs">
                        {p.data_fecho && (
                          <div>
                            <span className="text-white/25 tracking-wider block">DATA EVENTO</span>
                            <span className="text-white/60">{fmt(p.data_fecho)}</span>
                          </div>
                        )}
                        {p.local && (
                          <div>
                            <span className="text-white/25 tracking-wider block">LOCAL</span>
                            <span className="text-white/60 truncate block">{p.local}</span>
                          </div>
                        )}
                        {p.responsavel && (
                          <div>
                            <span className="text-white/25 tracking-wider block">RESPONSÁVEL</span>
                            <span className="text-white/60">{p.responsavel}</span>
                          </div>
                        )}
                        {p.email && (
                          <div>
                            <span className="text-white/25 tracking-wider block">EMAIL</span>
                            <a href={`mailto:${p.email}`} className="text-gold/60 hover:text-gold transition-colors truncate block">{p.email}</a>
                          </div>
                        )}
                        {p.telefone && (
                          <div>
                            <span className="text-white/25 tracking-wider block">TELEFONE</span>
                            <a href={`tel:${p.telefone}`} className="text-white/60 hover:text-white transition-colors">{p.telefone}</a>
                          </div>
                        )}
                      </div>
                    </div>
